...
%    [Pxx, f] = pwelch(dat, hamming(2^10), 2^9, 2^11, fs); % estimate PSD
    [Pxx, f] = pwelch(dat, hamming(2^9), 2^8, 2^10, fs); % estimate PSD
    fr = find(Pxx==max(Pxx)); 
    ...
    [Pxx, f] = pwelch(dat, hamming(2^10), 2^9, 2^11, fs);
   ...
   while outliers ~= 0
    ...
    block_no = floor((N/olap - (epl-olap)/olap)); NFFT = 2^ceil(log2(2*epl));
    ...
    th1 = quantile(log(amp+1), 100); th1 = exp(th1)-1;
    r1 = zeros(1,length(th1));
    for zz = 1:length(th1)
